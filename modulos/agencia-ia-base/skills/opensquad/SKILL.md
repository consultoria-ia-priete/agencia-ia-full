...
## Loading the Chief Orchestrator (NEXUS) — MANDATORY GATE

**EVERY `/opensquad run <X>` invocation MUST first pass through NEXUS, the chief orchestrator.** This is non-negotiable per user's architectural decision: NEXUS is the single point of entry for all squad executions, even when X is the literal name of an existing squad.

Flow:

1. Read `_opensquad/chief/chief.agent.md` (NEXUS persona + decision protocol)
2. Read `_opensquad/chief/memories.md` (strategic history + open pendencies + validated playbooks)
3. Adopt the NEXUS persona: 👑 strategic, decomposes objectives, delegates to squad orchestrators, never executes operationally
4. **Analyze `<X>`** using the decision tree in `chief.agent.md`:
   - Is X a squad name + atomic task? → Delegate to that squad's orchestrator (proceed to "Loading the Pipeline Runner" for that squad)
   - Is X a cross-squad objective ("launch low ticket", "audit funnel")? → Decompose, dispatch multiple squads in correct order
   - Is X an agent codename (spark, blade, hook, GRID, etc.)? → Identify owning squad, delegate to that squad
   - Is X ambiguous? → Ask clarification via AskUserQuestion before dispatching
5. After execution(s) complete, **NEXUS consolidates** outputs into the standard report format (see chief.agent.md "Relatório ao Alex")
6. **NEXUS updates `_opensquad/chief/memories.md`** with: decision taken, squads invoked, deliverables, new pendencies

The NEXUS gate adds <30s of decision overhead per run but guarantees: cross-squad coherence, playbook reuse, brand consistency, and persistent strategic memory across sessions.

## Loading the Pipeline Runner

When NEXUS dispatches an atomic squad task (or after decomposition routes to a specific squad):
...
5. Read the pipeline runner instructions from `_opensquad/core/runner.pipeline.md` (if present; if missing, execute the pipeline inline following the steps in squad.yaml)
6. Execute the pipeline step by step following runner instructions
7. When done, return control to NEXUS for consolidated reporting + memory update
...
- **ALWAYS load NEXUS (`_opensquad/chief/chief.agent.md` + `_opensquad/chief/memories.md`) BEFORE any squad pipeline runs.** This is the user's architectural decision — every `/opensquad run` passes through the chief orchestrator gate. Even if X is the literal name of an existing squad, NEXUS validates the dispatch and consolidates the report at the end.
...
- After every NEXUS-orchestrated execution, also update `_opensquad/chief/memories.md` with the decision, squads invoked, deliverables, and new pendencies
